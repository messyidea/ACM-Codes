...
void initfa() {
    dfs(1, -1, 0);      //dep and size
    for(int k = 0; k < MAX_LOG - 1; ++k) {  //构造倍增
        for(int u = 1; u <= n; ++u) {
...
int upslope(int u, int p) {     //u的第K父亲
...
Node get_middle(int a, int b, int ab) {
    ...
    if(dep[a] >= dep[b]) {
        return Node(1, upslope(a, (len - 1) / 2));  //下
    } else {
        return Node(2, upslope(b, len / 2));    //上
    }
}
