...
void reverse2(int u, int v) {
    while (pathf[u] != pathf[v]) {
        if (dep[pathf[u]] < dep[pathf[v]]) swap(u, v);
        update(light[pathf[u]], 0, tid[u]+1);           //对轻边进行修改
        update(weight[pathf[u]], tid[u]+1, tid[u]+2);   //a的后继节点的变色
        u = pathf[u];
        col[u] ^= 1;        //轻边的修改。
        u = fa[u];
    }
    if (tid[u] > tid[v]) swap(u, v);        //u在顶部
    update(light[pathf[u]], tid[u], tid[v]+1);  //翻转部分的修改
    if (u == pathf[u]) {    //如果u是重链的顶端，则把上面的轻链修改
        col[u] ^= 1;
    }else {
        update(weight[pathf[u]], tid[u], tid[u]+1);     //否则，还要先把重链修改
    }
    update(weight[pathf[u]], tid[v]+1, tid[v]+2);   //v底部修改，重链总是会到底的，所以不用考虑底部是否是轻链
}
...
int sum(int u, int v) {
    ...
    while (pathf[u] != pathf[v]) {
        if (dep[pathf[u]] < dep[pathf[v]]) swap(u, v);
        re += query(weight[pathf[u]], 1, tid[u]+1);     //加上重链
        u = pathf[u];
        re += cal(u);           //轻链是否改变。
        u = fa[u];
        cout<<"u == "<<u<<endl;
    ...
    if (tid[u] > tid[v]) swap(u, v);
    re += query(weight[pathf[u]], tid[u]+1, tid[v]+1);      //加上重链。
    return re;
}
...
int main() {
    ...
    while (T--) {
        ...
        dfs(0);         //求出了所有son
        dfs1(0, -1, 0, 0, 0);//建树完毕
        //for(int i=0;i<n;++i) cout<<i<<" "<<fa[i]<<" "<<dep[i]<<endl;
        m = in();
        while (m--) {
